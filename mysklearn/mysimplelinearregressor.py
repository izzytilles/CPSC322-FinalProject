...
class MySimpleLinearRegressor:
    ...
    def fit(self, X_train, y_train):
        ...
        self.slope, self.intercept = MySimpleLinearRegressor.compute_slope_intercept(
            X_train, y_train
        )
    ...
    def compute_slope_intercept(x, y):
        ...
        mean_x = np.mean(x)
        mean_y = np.mean(y)
        m = sum((x[i] - mean_x) * (y[i] - mean_y) for i in range(len(x))) / sum(
            (x[i] - mean_x) ** 2 for i in range(len(x))
        )
        # y = mx + b => y - mx
        b = mean_y - m * mean_x
        return m, b
